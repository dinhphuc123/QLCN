...
// ==============================================================================
// 5. GOOGLE GEMINI AI THẬT (REAL AI INTEGRATION)
// ==============================================================================
app.post('/api/ai/generate', requireAuth, async (req, res) => {
  try {
    const apiKey = process.env.GEMINI_API_KEY;
    const { student, reportType, customPrompt } = req.body;

    if (!student) {
      return res.status(400).json({ error: 'Thiếu thông tin học sinh để AI đánh giá.' });
    }

    // If API key is not configured, give a clear helpful notice
    if (!apiKey || apiKey.includes('your-gemini-api-key')) {
      return res.status(200).json({
        success: true,
        isMock: true,
        content: `[THÔNG BÁO HỆ THỐNG]: Chưa cấu hình GEMINI_API_KEY trên Vercel / Environment Variables.\n\n` +
          `👉 Để kích hoạt tính năng AI phân tích chuyên sâu tự động, vui lòng thêm GEMINI_API_KEY vào biến môi trường.\n\n` +
          `--- DỮ LIỆU ĐÁNH GIÁ SƠ BỘ CHO EM: ${student.name} (Lớp 12.7 - KTX ${student.dormRoom || 'A1-11'}) ---\n` +
          `- Điểm thi đua nề nếp: ${student.points || 100}/100\n` +
          `- Điểm trung bình năm trước: ${student.prevGPA || 7.5} (${student.prevRank || 'Khá'})\n` +
          `- Định hướng: Khối thi C00/D01 hoặc Hệ Dự Bị Đại Học Dân Tộc.`
      });
    }
    ...
    const systemPrompt = `Bạn là Trợ lý Sư phạm AI đắc lực của Cô Đỗ Kim Tuyền - Giáo viên chủ nhiệm lớp 12.7 trường Phổ thông Dân tộc Nội trú. Hãy phân tích hồ sơ học sinh sau đây và đưa ra nhận xét chính xác, giàu tính sư phạm, khuyến khích sự tiến bộ của học sinh:\n\n` +
      ...
      `- Tổ: ${student.group || 'Tổ 1'} | Phòng KTX: ${student.dormRoom || 'KTX'}\n` +
      `- Chức vụ: ${student.position || 'Thành viên'}\n` +
      `- Điểm thi đua hiện tại: ${student.points || 100}/100\n` +
      `- Điểm TB năm trước (lớp 11): ${student.prevGPA || 7.0} (Xếp loại: ${student.prevRank || 'Khá'})\n` +
      `- Hạnh kiểm: ${student.prevConduct || 'Tốt'}\n` +
      `- Hoàn cảnh: ${student.isPoor ? 'Học sinh diện chính sách / hộ cận nghèo cần quan tâm' : 'Bình thường'}\n` +
      `- Ghi chú GVCN: ${student.note || 'Không có'}\n\n` +
      `Yêu cầu: ${promptInstruction}\n` +
      (customPrompt ? `Yêu cầu bổ sung của GVCN: ${customPrompt}` : '');

    // Call Google Gemini API (gemini-2.5-flash / fallback to gemini-1.5-flash)
    ...
    const aiText = result.candidates?.[0]?.content?.parts?.[0]?.text || 'Không có phản hồi từ AI.';

    addAuditLog(req.user, 'TRỢ LÝ AI TỔNG HỢP', `${student.name} (${reportType})`);
    return res.json({ success: true, content: aiText, isMock: false });
  } catch (err) {
    console.error('AI generate exception:', err.message);
    return res.status(500).json({ success: false, error: `Lỗi kết nối Gemini AI: ${err.message}` });
  }
});

// ==============================================================================
// 6. CORE DATA ENDPOINTS (Direct Supabase Cloud / Fallback Demo)
// ==============================================================================
